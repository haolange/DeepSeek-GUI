...
import { Code2, FolderOpen, PanelsTopLeft } from 'lucide-react'
...
type DesignSettingsTab = 'code' | 'canvas' | 'workspace'
...
export function DesignSettingsSection({ ctx }: { ctx: Record<string, unknown> }): ReactElement {
  ...
  const [activeTab, setActiveTab] = useState<DesignSettingsTab>('code')

  return (
    <div className="space-y-5">
      <SettingsTabs
        baseId="design-settings"
        ariaLabel={t('design')}
        items={[
          { id: 'code', label: t('designSettingsCode'), icon: Code2 },
          { id: 'canvas', label: t('designSettingsCanvas'), icon: PanelsTopLeft },
          { id: 'workspace', label: t('designSettingsWorkspace'), icon: FolderOpen }
        ]}
        value={activeTab}
        onChange={setActiveTab}
      />

      <SettingsTabPanel baseId="design-settings" tabId="code" active={activeTab === 'code'}>
      ...
      <SettingsTabPanel baseId="design-settings" tabId="canvas" active={activeTab === 'canvas'}>
      ...
      <SettingsTabPanel baseId="design-settings" tabId="workspace" active={activeTab === 'workspace'}>
